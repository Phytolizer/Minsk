...
static void bound_global_scope_vector_init(
...
static void bound_global_scope_vector_push(
    bound_global_scope_vector_t* vector, bound_global_scope_t value) {
...
static bound_global_scope_t bound_global_scope_vector_pop(
    bound_global_scope_vector_t* vector) {
...
static void bound_global_scope_vector_free(
    bound_global_scope_vector_t* vector) {
  free(vector->data);
...
bound_global_scope_t binder_bind_global_scope(
  ...
  bound_global_scope_init(
      &global_scope, NULL, diagnostics, variables, expression);
  binder_free(&binder);
  return global_scope;
}

static bound_expression_t* bind_binary_expression(
    binder_t* binder, const binary_expression_syntax_t* syntax) {
  ...
  const bound_binary_operator_t* op =
      bound_binary_operator_bind(syntax->operator_token.base.kind,
          bound_expression_type(left), bound_expression_type(right));
  if (op == NULL) {
    diagnostic_bag_report_undefined_binary_operator(&binder->diagnostics,
        token_span(&syntax->operator_token), syntax->operator_token.text,
        bound_expression_type(left), bound_expression_type(right));
  }
...
static bound_expression_t* bind_literal_expression(
    binder_t* binder, const literal_expression_syntax_t* syntax) {
  (void)binder;
  bound_expression_t* expression =
...
static bound_expression_t* bind_assignment_expression(
    binder_t* binder, const assignment_expression_syntax_t* syntax) {
  bound_expression_t* expression =
...
static bound_expression_t* bind_name_expression(
    binder_t* binder, const name_expression_syntax_t* syntax) {
  (void)binder;
  variable_symbol_t* variable =
      bound_scope_try_lookup(binder->scope, syntax->identifier_token.text);
  if (variable == NULL) {
    diagnostic_bag_report_undefined_variable(&binder->diagnostics,
        token_span(&syntax->identifier_token), syntax->identifier_token.text);
...
static bound_expression_t* bind_parenthesized_expression(
    binder_t* binder, const parenthesized_expression_syntax_t* syntax) {
  bound_expression_t* expression =
      binder_bind_expression(binder, syntax->expression);
  return expression;
}
...
static bound_expression_t* bind_unary_expression(
    binder_t* binder, const unary_expression_syntax_t* syntax) {
  ...
  if (op == NULL) {
    diagnostic_bag_report_undefined_unary_operator(&binder->diagnostics,
        token_span(&syntax->operator_token), syntax->operator_token.text,
...
bound_expression_t* binder_bind_expression(
  ...
  case syntax_kind_name_expression:
    return bind_name_expression(
        binder, (const name_expression_syntax_t*)expression);
  ...
  case syntax_kind_unary_expression:
    return bind_unary_expression(
        binder, (const unary_expression_syntax_t*)expression);
  default:
